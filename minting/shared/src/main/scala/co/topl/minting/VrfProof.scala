...
import co.topl.algebras.{Cache, ClockAlgebra, UnsafeResource}
...
object VrfProof {
  ...
  object Eval {
    ...
    def make[F[_]: MonadError[*[_], Throwable]: Sync: Logger: Parallel](
      ...
      vrfConfig:                VrfConfig,
      vrfProofsCache:           F[Cache[F, LongMap[Proofs.Knowledge.VrfEd25519]]],
      rhosCache:                F[Cache[F, LongMap[Rho]]]
    ): F[VrfProofAlgebra[F]] =
      (vrfProofsCache, rhosCache).mapN((vrfProofsCache, rhosCache) =>
        new Impl[F](
          skVrf,
          clock,
    ...
    private class Impl[F[_]: MonadError[*[_], Throwable]: Sync: Logger: Parallel](
      ...
      ed25519VRFResource:       UnsafeResource[F, Ed25519VRF],
      vrfConfig:                VrfConfig,
      vrfProofsCache:           Cache[F, LongMap[Proofs.Knowledge.VrfEd25519]],
      rhosCache:                Cache[F, LongMap[Rho]]
      ...
      def precomputeForEpoch(epoch: Epoch, eta: Eta): F[Unit] =
        for {
          ...
          _ <- (vrfProofsCache.put(eta.show)(vrfProofs), rhosCache.put(eta.show)(rhoValues)).tupled
        } yield ()

      def proofForSlot(slot: Slot, eta: Eta): F[Proofs.Knowledge.VrfEd25519] =
        OptionT(vrfProofsCache.get(eta.show))
      ...
      def rhoForSlot(slot: Slot, eta: Eta): F[Rho] =
        OptionT(rhosCache.get(eta.show))
          .subflatMap(_.get(slot))
          .getOrElseF(
