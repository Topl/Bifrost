...
import co.topl.catsakka.{AkkaCatsRuntime, SourceMatNotUsed}
...
object BramblTetra extends IOApp.Simple {
  ...
  private def infiniteTransactions(
    sleepDuration:      FiniteDuration
  )(implicit rpcClient: ToplRpc[F, SourceMatNotUsed], logger: Logger[F]) =
    Sync[F]
      .defer(
        for {
  ...
  implicit class TransactionOps(transaction: Transaction) {
    ...
    def broadcast[F[_]: ToplRpc[*[_], Source[*, NotUsed]]]: F[Unit] =
      implicitly[ToplRpc[F, Source[*, NotUsed]]].broadcastTransaction(transaction)
  }
