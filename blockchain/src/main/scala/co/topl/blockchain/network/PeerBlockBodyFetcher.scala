...
object PeerBlockBodyFetcher {
  ...
  private def downloadBlocks[F[_]: Applicative](state: State[F], blocksToDownload: NonEmptyChain[TypedIdentifier]) = {
  ...
  private def stopActor[F[_]: Applicative](state: State[F]) =
    (state, state).pure[F]
}
