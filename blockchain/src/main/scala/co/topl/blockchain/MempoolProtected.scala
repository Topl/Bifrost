...
class MempoolProtected[F[_]: Async: Logger](
  ...
  override def add(transactionId: TransactionId): F[Boolean] =
    for {
      ...
      _             <- if (res) underlying.add(transactionId) else ().pure[F]
    } yield res

  override def remove(transactionId: TransactionId): F[Unit] = underlying.remove(transactionId)
  ...
  private def checkTransaction(
    ...
    if (!config.enabled || meta.txCount < config.noCheckIfLess) {
      Logger[F].info(show"Skip tx verification: ${meta.txCount} txs in mempool") >>
      true.pure[F]
    } else {
      EitherT
        ...
        .value
        .recoverWith { case e =>
          Logger[F].error(e)(show"Error during checking transaction in Mempool") >>
        ...
        .flatMap {
          _.fold(
            ...
            _ => Logger[F].info(show"Added transaction $txId to mempool") >> true.pure[F]
          )
        }
    }
  ...
  private def doSemanticCheck(currentHeader: BlockHeader, graph: MempoolGraph, meta: MempoolMetadata)(
    ...
    EitherT(semanticValidationAlgebra.validate(semanticContext)(txToValidate).map(_.toEither.leftMap(_.mkString_(","))))
  }
