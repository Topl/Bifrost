...
import co.topl.genus.services.transaction_query.{QueryTxReq, TransactionQuery}
...
class TransactionQueryServiceSpec extends AsyncFlatSpec with AsyncMockFactory with Matchers {
  ...
  it should "return an empty list of transactions when the database is empty" in {
    ...
    val queryTxReq: QueryTxReq = QueryTxReq(queryFilter.some)
    ...
    val underTest: TransactionQuery =
      TransactionQueryService.Eval.make(databaseClient, 5.seconds)
  ...
  it should "return all existing transactions when the database is not empty and no filters are provided" in {
    val queryTxReq: QueryTxReq = QueryTxReq(None)
