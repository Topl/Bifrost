        ...
        def delayedUntilSlot(slot: Slot): F[Unit] =
          Async[F].fromFuture(
            globalSlot
              ...
              .map(delay =>
                if (delay.toMillis > 0) akka.pattern.after(delay, system.classicSystem.scheduler)(Future.unit)
                else Future.unit
              )
          )
          ...
          Async[F].fromFuture(
            currentTimestamp
              .map(currentTimestamp =>
                akka.pattern.after((timestamp - currentTimestamp).millis, system.classicSystem.scheduler)(Future.unit)
              )
          )
      }
  }
