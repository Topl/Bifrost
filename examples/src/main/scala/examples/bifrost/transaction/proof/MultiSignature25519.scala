package examples.bifrost.transaction.proof

import com.google.common.primitives.Ints
import examples.bifrost.transaction.box.proposition.MofNProposition
import scorex.core.serialization.Serializer
import scorex.core.transaction.box.proposition.{Constants25519, Proposition, PublicKey25519Proposition}
...
object MultiSignature25519Serializer extends Serializer[MultiSignature25519] {
  override def toBytes(obj: MultiSignature25519): Array[Byte] =
    Ints.toByteArray(obj.signatureSet.size) ++ obj.signatureSet.foldLeft(Array[Byte]())((total, sig) => total ++ sig.signature)
  ...
  override def parseBytes(bytes: Array[Byte]): Try[MultiSignature25519] = Try {
    ...
    val signatureSet: Set[Signature25519] = (0 until numSignatures).map {
      i => Signature25519(bytes.slice(Ints.BYTES + i*MultiSignature25519.SignatureSize, Ints.BYTES + (i + 1)*MultiSignature25519.SignatureSize))
    }.toSet
