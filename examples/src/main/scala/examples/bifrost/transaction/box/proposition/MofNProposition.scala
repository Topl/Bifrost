package examples.bifrost.transaction.box.proposition
...
object MofNPropositionSerializer extends Serializer[MofNProposition] {
  override def toBytes(obj: MofNProposition): Array[Byte] = Bytes.concat(
    ...
    obj.setOfPubKeyBytes.foldLeft(Array[Byte]())((a: Array[Byte], b: Array[Byte]) => a ++ b)
  )
  ...
  override def parseBytes(bytes: Array[Byte]): Try[MofNProposition] = Try {
    ...
    val setPubKeys = (0 until n).map {
      i => bytes.slice(2*Ints.BYTES + i*Constants25519.PubKeyLength, 2*Ints.BYTES + (i + 1)*Constants25519.PubKeyLength)
