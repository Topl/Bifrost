...
import co.topl.consensus.algebras.{EtaCalculationAlgebra, LocalChainAlgebra}
  ...
  private def traverseEpoch[F[_]: MonadError[*[_], Throwable]: Logger: Parallel: Sync](
    ...
    (boundary.start: Long)
      .iterateUntilM(localSlot =>
        for {
          _ <- processSlot(localSlot, mints, headerStore, blockStore, localChain, ed25519VrfResource)
        } yield localSlot + 1
      )(_ >= boundary.end)
      .void
  ...
  private def processSlot[F[_]: MonadError[*[_], Throwable]: Logger: Parallel: Sync](
    ...
    for {
      ...
      _ <- mintedBlocks.traverse(
        processMintedBlock(_, blockStore, localChain, ed25519VrfResource, canonicalHead)
      )
    } yield ()
    ...
    for {
      ...
      _ <-
        if (localChainIsWorseThan)
          EitherT
            .pure[F, BlockHeaderV2](nextBlock.headerV2)
