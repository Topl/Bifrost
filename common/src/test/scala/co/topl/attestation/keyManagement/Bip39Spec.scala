...
class Bip39Spec extends AnyPropSpec with CommonGenerators {
  case class Bip39TestVector(mnemonic: String, seed: String)
  ...
  val englishBip39: Bip39 =
    Bip39.withLanguage(Bip39.English).getOrElse(throw new Error("Failed to read english word list"))
  ...
  property("test vectors should pass") {
    testVectors.foreach { vector =>
      val seed = englishBip39.phraseToSeed(vector.mnemonic, Some("TREZOR"))
