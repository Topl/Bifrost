...
object ModifierId extends BifrostSerializer[ModifierId] {
  ...
  def apply(nodeViewModifier: NodeViewModifier): ModifierId = nodeViewModifier match {
    case mod: Block          => new ModifierId(Block.modifierTypeId +: Hash(mod.messageToSign).bytes)
    case mod: Transaction.TX => new ModifierId(Transaction.modifierTypeId +: Hash(mod.messageToSign).bytes)
