...
case class UnconfirmedTransactionDataModel(
  txType:          String,
  timestamp:       String,
...
object UnconfirmedTransactionDataModel {

  def apply(tx: Transaction[_, _ <: Proposition]): UnconfirmedTransactionDataModel =
    tx match {
      case polyTransfer: PolyTransfer[_] =>
        UnconfirmedTransactionDataModel(
