...
case class HdKesKeyFile(kes_info: CipherInfo, fileName: String, oldFileName: String) {
  import HdKesKeyFile._

  /**
   * Decrypt the HdKesScheme  from kes_info cipher information
   * @param password pass phrase used in AES encryption scheme
   * @return
   */
  def getKey(password: String): Try[HdKesScheme] = decryptKesSK(password)

  private def decryptKesSK(password: String): Try[HdKesScheme] = Try {
    ...
    val decryptedKey = HdKesScheme.deserialize(decrypted)
    decryptedKey
  }
...
object HdKesKeyFile {
  ...
  def newKeyFile(
    ...
    val fileName =
      s"$defaultKeyDir/$dateString-${Base58.encode(???)}.json"
  ...
  def updateKeyFile(
    ...
    val newKeyFile = HdKesKeyFile(kes_info, fileName, keyFile.fileName)
  ...
  def readFile(filename: String): HdKesKeyFile = {
  ...
  def restore(storageDir: String): Option[HdKesKeyFile] = {
  ...
  implicit private val decodeKeyFile: Decoder[HdKesKeyFile] = (c: HCursor) =>
    for {
      ...
      new HdKesKeyFile(kes_info, fileName, oldFileName)
    }
}
