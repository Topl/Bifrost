...
case class InternalNode[H : Hash](left: Node, right: Node) extends Node {
  ...
  override def toString: String = s"InternalNode(" +
    s"left: ${Base16.encode(left.hash.toBytes)}, " +
    s"right: ${if (right.hash.toBytes.isEmpty) "null" else Base16.encode(right.hash.toBytes)}," +
    s"hash: ${Base16.encode(hash.toBytes)})"
}
...
case class Leaf[H : Hash](data: LeafData) extends Node {
  ...
  override def toString: String = s"Leaf(${Base16.encode(hash.toBytes)})"
}
...
case class EmptyRootNode[H : Hash]() extends Node {
  ...
  override def toString: String = s"EmptyRootNode(${Base16.encode(hash.toBytes)})"
}
