 ...
 */
case class InternalNode[H, D : BytesOf](left: Node[D], right: Node[D])(implicit
                                                                       hashFunc: Hash[H, D]) extends Node[D] {

  override lazy val hash: D = hashFunc.hash(
    MerkleTree.InternalNodePrefix,
    BytesOf[D].concat(left.hash, right.hash))
