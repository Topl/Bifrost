...
import co.topl.crypto.generation.mnemonic.{Entropy, FromEntropy, Language, MnemonicSize}
...
class MnemonicSpec extends AnyPropSpec with ScalaCheckPropertyChecks with ScalaCheckDrivenPropertyChecks {
  ...
  property("mnemonic should generating entropy from test vector 1") {
    ...
    val entropy: Entropy = FromEntropy.derive(specIn.words, specIn.size, specIn.language)(e => e) match {
