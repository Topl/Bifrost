...
class Curve25519AxolotlSpec extends AnyPropSpec with ScalaCheckDrivenPropertyChecks with Matchers {
  ...
  /* The tests below are generated on June 7, 2021 from co.topl.crypto.signing.Curve25519.scala:
   * https://github.com/Topl/Bifrost/blob/ef396e2faa36cfb50a6ec00961a2a021491c88a6/crypto/src/main/scala/co/topl/crypto/signatures/Curve25519.scala
   * Commit hash: ef396e2faa36cfb50a6ec00961a2a021491c88a6
   */
  property("test vector - 1") {
    val curve25519 = new Curve25519
    val specInSk = SecretKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("184F0E9851971998E732078544C96B36C3D01CEDF7CAA332359D6F1D83567054")))
    )
    val specInMsg = Bytes(Hex.decode(""))

    val vk = curve25519.generatePublicKey(specInSk)
    val sig = curve25519.sign(specInSk, specInMsg)

    val specOutVk = VerificationKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("4652486EBC271520D844E5BDDA9AC243C05DCBE7BC9B93807073A32177A6F73D")))
    )
  ...
  property("test vector - 2, one byte message length") {
    val curve25519 = new Curve25519
    val specInSk = SecretKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("60303AE22B998861BCE3B28F33EEC1BE758A213C86C93C076DBE9F558C11C752")))
    )
    ...
    val specOutVk = VerificationKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("FFBC7BA2E4C43BE03F8A7F020D0651F582AD1901C254EEBB4EC2ECB73148E50D")))
    )
    val specOutSig = Proofs.Signature.Curve25519(
      Sized.strictUnsafe(
        Bytes(
  ...
  property("test vector - 3, two bytes message length") {
    val curve25519 = new Curve25519
    val specInSk = SecretKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("F861A03AF4F77D870FC21E05E7E80678095C92D808CFB3B5C279EE04C74ACA53")))
    )
    ...
    val specOutVk = VerificationKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("59DF714EAD8FB10B68E31153AD01994117652CB3C960C6E32C57E7DEC28A5846")))
    )
    val specOutSig = Proofs.Signature.Curve25519(
      Sized.strictUnsafe(
        Bytes(
  ...
  property("test vector - 4, 1023 bytes message length") {
    val curve25519 = new Curve25519
    val specInSk = SecretKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("30929AC0656504EF9782D0164A3603C09AA3FDCB07973948D03472A571CAB769")))
    )
    val specInMsg = Bytes(
      Hex.decode(
    ...
    val specOutVk = VerificationKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("05C54041990DEF35C6B163F1B6DBE5194BC4139C7D3BA8F1FCF28DE3A8B07357")))
    )
    val specOutSig = Proofs.Signature.Curve25519(
      Sized.strictUnsafe(
        Bytes(
  ...
  property("test vector - 5, abc hashed by SHA512 as message") {
    val curve25519 = new Curve25519
    val specInSk = SecretKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("C8A312FBBCF8FF3213B917D4232BCA39AAE7740338791114072F07FF3692CA72")))
    )
    ...
    val specOutVk = VerificationKeys.Curve25519(
      Sized.strictUnsafe(Bytes(Hex.decode("5EFBB8396F73F2663403B242347E8B72FBA96DD1E92DBA71A909076643631752")))
    )
    val specOutSig = Proofs.Signature.Curve25519(
      Sized.strictUnsafe(
        Bytes(
