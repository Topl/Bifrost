...
object GenusServerApp
  ...
  private def applicationResource: Resource[F, Unit] =
    for {
      ...
      orientdb       <- OrientDBFacade.make[F](appConfig.orientDbDirectory, appConfig.orientDbUser, appConfig.orientDbPassword)
      headerInserter <- GraphHeaderInserter.make[F](orientdb.getTx)
