...
import co.topl.components.Serializer
import co.topl.primitives.{ActorRefWrapper, _}
import co.topl.remote.SpecTypes._
...
class Router(seed:Array[Byte], inputRef:Seq[ActorRefWrapper],settings:AppSettings) extends Actor
  ...
  private def messageFromPeer: Receive = {
    case msg @ Message(spec, Left(msgBytes), Some(remote)) =>
      def processRemoteHolders:Unit = {
        spec.parseBytes(msgBytes) match {
          case Success(msg:HoldersType@unchecked) => Try{
            for (string<-msg._1) {
        ...
        case 0 =>
          spec.messageCode match {
            case DiffuseDataSpec.messageCode =>
              spec.parseBytes(msgBytes) match {
                case Success(value:(Mac,Array[Byte])@unchecked) => Try{
                  val mac = value._1
