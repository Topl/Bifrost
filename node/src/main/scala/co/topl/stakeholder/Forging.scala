...
import co.topl.stakeholder.primitives.{ForgingKey, Keys, Ratio, SharedData, Types}
...
trait Forging extends Members with Types {
  ...
  def forgeBlock(forgerKeys: Keys): Unit = if (!SharedData.limiterFlag) Try {
    ...
    val pi_y: Pi = vrf.vrfProof(
      forgerKeys.sk_vrf,
      eta ++ serializer.getBytes(slot) ++ serializer.getBytes("TEST")
    )
    ...
    def testThenForge(test: Rho, thr: Ratio): Unit = if (compare(test, thr)) {
      ...
      val pi: Pi = vrf.vrfProof(
        forgerKeys.sk_vrf,
        eta ++ serializer.getBytes(slot) ++ serializer.getBytes("NONCE")
      )
      ...
      updateLocalState(localState, (slot, block.id)) match {
        case Some(forgedState: State) =>
          send(
            selfWrapper,
            rng
              .shuffle(holders.filter(_ != selfWrapper))
              .take(numGossipersForge),
            SendBlock(block, selfWrapper)
          )
