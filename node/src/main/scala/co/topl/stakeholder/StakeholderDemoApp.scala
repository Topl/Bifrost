...
class StakeholderDemoApp(startupOpts: StartupOpts) extends NodeLogging with Runnable {
  ...
  private val routerRef:ActorRef =
    actorSystem.actorOf(
      Router.props(blake2b256.hash(java.util.UUID.randomUUID.toString.getBytes).value,Seq(networkControllerRef,peerManagerRef),settings),
      "Remote"
    )

  private val stakeholderRef: ActorRef =
    actorSystem.actorOf(
      Stakeholder.props(blake2b256.hash(java.util.UUID.randomUUID.toString.getBytes).value,0,Seq(routerRef),settings),
      "Stakeholder"
    )
