...
import co.topl.stakeholder.primitives.{ByteStream, Fch, LDBStore, SharedData, TetraParameters, Types}
...
class StateStorage(dir: String, serializer: Serializer) extends Types {
  ...
  private val stateStoreCache: LoadingCache[BigInt, DB] = CacheBuilder
    .newBuilder()
  ...
  private val etaStoreCache: LoadingCache[BigInt, DB] = CacheBuilder
    .newBuilder()
    .maximumSize(dbCacheSize)
    .removalListener { (notification: RemovalNotification[BigInt, DB]) =>
      notification.getValue.close()
  ...
  private val epochStakeDistCache: LoadingCache[SlotId, (State, Eta)] = CacheBuilder
    .newBuilder()
    .maximumSize(4)
  ...
  private val stateCache: LoadingCache[SlotId, (State, Eta)] = CacheBuilder
    .newBuilder()
  ...
  def add(id: SlotId, ls: State, eta: Eta): Unit = {
    if (!known(id)) {
      stateStoreCache
        .get(id._1 / one_ninth_epoch)
        .update(Seq(), Seq(id._2 -> ByteArrayWrapper(serializer.getBytes(ls))))
