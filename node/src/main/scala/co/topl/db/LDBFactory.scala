...
case class StoreRegistry(factory: DBFactory) extends DBFactory with Logging {
  ...
  private def add(file: File, create: => DB): DB = {
    lock.writeLock().lock()
    try map.getOrElseUpdate(file, RegisteredDB(create, file))
  ...
  def open(path: File, options: Options): DB = {
    lock.writeLock().lock()
    try add(path, factory.open(path, options))
    catch {
      case x: Throwable =>
        log.error(
...
object LDBFactory extends Logging {
  ...
  private def loadFactory(loader: ClassLoader, factoryName: String): Option[DBFactory] =
    try Some(loader.loadClass(factoryName).getConstructor().newInstance().asInstanceOf[DBFactory])
      ...
      case e: Throwable =>
        log.warn(
          s"Failed to load database factory $factoryName. Classloader is $loader.\nClasspath is ${System.getProperty("java.class.path")}",
          e
        )
        None
    }
