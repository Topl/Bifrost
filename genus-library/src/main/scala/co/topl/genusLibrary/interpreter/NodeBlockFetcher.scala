  ...
  def make[F[_]: Async: Logger](
    ...
    Resource.pure {
      ...
      new NodeBlockFetcherAlgebra[F, Stream[F, *]] {
        ...
        override def fetch(height: Long): F[Either[GRE, Option[BlockData]]] =
          toplRpc
            ...
            .flatMap {
              ...
              case None =>
                Option.empty[BlockData].asRight[GRE].pure[F]
            }

        // TODO: TSDK-186 | Do calls concurrently.
        private def fetchBlock(blockId: BlockId, toplRpc: ToplRpc[F, Stream[F, *]]): F[Either[GRE, BlockData]] = (
          for {
            header       <- OptionT(toplRpc.fetchBlockHeader(blockId)).toRight(GREs.HeaderNotFound(blockId): GRE)
            body         <- OptionT(toplRpc.fetchBlockBody(blockId)).toRight(GREs.BodyNotFound(blockId): GRE)
        ...
        private def fetchTransactions(
          body:    BlockBody,
          toplRpc: ToplRpc[F, Stream[F, *]]
        ): F[Either[GRE, Chain[IoTransaction]]] =
          ...
          } map [Either[GRE, Chain[IoTransaction]]] (_.left.map(
            GREs.NonExistentTransactions
          ))
