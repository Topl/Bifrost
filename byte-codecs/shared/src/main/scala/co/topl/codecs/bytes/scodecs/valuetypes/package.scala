...
package object valuetypes extends ValuetypesCodecs {
  ...
  def intTo4Bytes(int: Int): ByteVector =
    ByteVector(ByteBuffer.allocate(4).putInt(int))

  def longTo8Bytes(long: Long): ByteVector =
    ByteVector(ByteBuffer.allocate(8).putLong(long))
}
