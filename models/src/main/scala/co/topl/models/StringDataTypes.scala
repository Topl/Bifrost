...
object StringDataTypes {
  ...
  object Base58Data {
     ...
     */
    def fromData(bytes: Array[Byte]): Base58Data = bytes.coerce[Base58Data]
    ...
    def validated(from: String): DataEncodingValidationResult[Base58Data] =
      Base58.decode(from).leftMap(_ => InvalidCharacter).map(fromData).toValidatedNec
  ...
  object Base16Data {
     ...
     */
    def fromData(bytes: Array[Byte]): Base16Data = bytes.coerce[Base16Data]

    def validated(from: String): DataEncodingValidationResult[Base16Data] =
      Base16.decode(from).leftMap(_ => InvalidCharacter).map(fromData).toValidatedNec
