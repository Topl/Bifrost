...
class NodeViewHolder(override val settings: AppSettings, bifrostContext: BifrostContext)(implicit ec: ExecutionContext)
  ...
  lazy val modifierCompanions: Map[ModifierTypeId, BifrostSerializer[_ <: NodeViewModifier]] =
    Map(Block.modifierTypeId -> BlockSerializer, GenericTransaction.modifierTypeId -> TransactionSerializer)
...
object NodeViewHolder extends Logging {
  ...
  def initializeGenesis(settings: AppSettings): NodeView = {
    ...
    val arbTx = ArbitTransfer(
      IndexedSeq(genesisAccountPriv.publicImage -> 0L),
    ...
    assert(
      genesisBlock.encodedId == "9VX9smBd7Jz56HzTcmY6EZiLfrn7WdxECbsSgNRrPXmu",
    ...
    assert(
      !settings.walletSeed.startsWith("genesis") || wallet
        .boxes()
        .flatMap(_.box match {
...
object NodeViewHolderRef {
  ...
  def apply(settings: AppSettings, bifrostContext: BifrostContext)(implicit system: ActorSystem, ec: ExecutionContext): ActorRef =
    system.actorOf(props(settings, bifrostContext))

  def apply(name: String, settings: AppSettings, bifrostContext: BifrostContext)(implicit
    system: ActorSystem,
    ec: ExecutionContext
  ): ActorRef =
