...
trait TransferUtil {
    ...
    toReceive
      .foldLeft((IndexedSeq[(PrivateKey25519, Long, Long)](), IndexedSeq[(PublicKey25519Proposition, Long)]())) {
        case (a, (recipient, amount)) =>
          ...
          val keyFilteredBoxes: Seq[Box] = sender.flatMap(s => tbr.boxesByKey(s))
          ...
          val keyAndTypeFilteredBoxes: Seq[TokenBox] = txType match {
            ...
            case "AssetTransfer" =>
              if (extraArgs(2).asInstanceOf[Option[String]].isDefined) {
                keyFilteredBoxes.flatMap(_ match {
                  case a: AssetBox if Base58.encode(a.id) equals extraArgs(2).asInstanceOf[Option[String]].get =>
                    Some(a)
                })
              } else {
                keyFilteredBoxes.flatMap(_ match {
          ...
          val senderInputBoxes: IndexedSeq[(PrivateKey25519, Long, Long)] = keyAndTypeFilteredBoxes.flatMap { b =>
  ...
  def validateTx(tx: TransferTransaction): Try[Unit] = Try {
    ...
    require(tx.signatures.forall { case (prop, sign) =>
      sign.isValid(prop, tx.messageToSign)
    })
    require(tx.from.forall { case (prop, nonce) =>
      tx.signatures.contains(prop)
    })
