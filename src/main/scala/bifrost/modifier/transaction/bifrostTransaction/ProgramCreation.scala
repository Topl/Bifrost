...
//  lazy val proposition = MofNProposition(1, parties.map(_._1.pubKeyBytes).toSet)
  ...
  lazy val newStateBoxes: Traversable[StateBox] = {
    ...
    val stateBox =
      StateBox(owner, stateNonce, UUID.nameUUIDFromBytes(StateBox.idFromBox(owner, stateNonce)), executionBuilder.core.variables)
  ...
  override lazy val newBoxes: Traversable[Box] = {
    ...
    val codeBox = CodeBox(
      owner,
      codeNonce,
      UUID.nameUUIDFromBytes(CodeBox.idFromBox(owner, codeNonce)),
      executionBuilder.core.code.values.toSeq,
      executionBuilder.core.interface
    )
    val stateUUIDs: Seq[UUID] = Seq(UUID.nameUUIDFromBytes(stateBox.id)) ++ readOnlyStateBoxes
    val executionBox = ExecutionBox(
      owner,
      execNonce,
      UUID.nameUUIDFromBytes(ExecutionBox.idFromBox(owner, execNonce)),
      stateUUIDs,
      Seq(codeBox.id)
    )
...
object ProgramCreation {
  ...
  implicit val decodeProgramCreation: Decoder[ProgramCreation] = (c: HCursor) =>
    for {
