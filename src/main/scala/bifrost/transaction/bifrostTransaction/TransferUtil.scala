...
//  }
  ...
  def parametersForApply(from: IndexedSeq[(PrivateKey25519, Nonce)],
                         ...
                         txType: String,
                         extraArgs: Any*):
  Try[(IndexedSeq[(PublicKey25519Proposition, Nonce)], IndexedSeq[(PublicKey25519Proposition, Signature25519)])] = Try {
    val fromPub = from.map { case (pr, n) => pr.publicImage -> n }
    //val fakeSigs = from.map(_ => Signature25519(Array()))

    val undersigned = txType match {
      case "PolyTransfer" => PolyTransfer(fromPub, to, IndexedSeq(), fee, timestamp, extraArgs(0).asInstanceOf[String])
      case "ArbitTransfer" => ArbitTransfer(fromPub, to, IndexedSeq(), fee, timestamp, extraArgs(0).asInstanceOf[String])
      case "AssetTransfer" => AssetTransfer(
        fromPub,
        to,
        IndexedSeq(),
    ...
    val msg = undersigned.messageToSign
    val sigs = from.map { case (priv, _) => (priv.publicImage, PrivateKey25519Companion.sign(priv, msg)) }
    (fromPub, sigs)
  }
