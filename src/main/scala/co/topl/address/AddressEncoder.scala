...
case class AddressEncoder(networkPrefix: NetworkPrefix) {
  ...
  def toString(address: ToplAddress): String = {
  ...
  def fromString(addressStr: String): Try[ToplAddress] = Base58.decode(addressStr).flatMap { bytes =>
    Try {
      require(bytes.length == encodedAddressLength, s"Invalid address: Not the required length")

      val network = bytes(0)
      val addressType = bytes(1)
      val propContent = bytes.slice(2, bytes.length - checksumLength)
      ...
      require(Blake2b256(propContent).take(checksumLength) sameElements checksum, s"Invalid address: Checksum fails for $addressStr")

      addressType match {
        case PublicKeyAddress.addressTypePrefix => new PublicKeyAddress(AddressContent @@ propContent)
