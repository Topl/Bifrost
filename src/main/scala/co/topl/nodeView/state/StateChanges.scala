...
case class StateChanges(override val boxIdsToRemove: Seq[BoxId],
                        override val toAppend: Seq[Box[_]],
...
object StateChanges {
  def apply(mod: Block): Try[StateChanges] = Try {
    ...
    mod.transactions.map {
      tx => (tx.boxIdsToOpen, tx.newBoxes)
    }.foldLeft((Seq[BoxId](), Seq[Box[_]]()))(
      (aggregate, boxDelta) => {
        (
