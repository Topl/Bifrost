...
//  with BifrostGenerators
  ...
  property("Appending version3 blocks after height = forkHeight should work") {
    ...
    for(i <- 2L to testSettings_version0.forkHeight) {
      val tempBlock = BifrostBlock(ForkSpec.history.bestBlockId,
        System.currentTimeMillis(),
        ArbitBox(PublicKey25519Proposition(ForkSpec.history.bestBlockId), 0L, 10000L),
        Signature25519(Array.fill(Curve25519.SignatureLength)(1: Byte)),
        Seq(),
        0L,
        testSettings_version0.version)

      ForkSpec.history = ForkSpec.history.append(tempBlock).get._1
      assert(ForkSpec.history.modifierById(tempBlock.id).isDefined)
    }

    val tempBlock_version3_1 = BifrostBlock(ForkSpec.history.bestBlockId,
      System.currentTimeMillis(),
      ArbitBox(PublicKey25519Proposition(ForkSpec.history.bestBlockId), 0L, 10000L),
      Signature25519(Array.fill(Curve25519.SignatureLength)(1: Byte)),
      Seq(),
    ...
    val tempBlock_version3_2 = BifrostBlock(ForkSpec.history.bestBlockId,
      System.currentTimeMillis(),
      ArbitBox(PublicKey25519Proposition(ForkSpec.history.bestBlockId), 0L, 10000L),
      Signature25519(Array.fill(Curve25519.SignatureLength)(1: Byte)),
      Seq(),
    ...
    ForkSpec.history = ForkSpec.history.append(tempBlock_version3_1).get._1
    assert(ForkSpec.history.modifierById(tempBlock_version3_1.id).isDefined)

    ForkSpec.history = ForkSpec.history.append(tempBlock_version3_2).get._1
    assert(ForkSpec.history.modifierById(tempBlock_version3_2.id).isDefined)

    //Height represents height of bestBlockId which is at 1 less than current number of blocks
    ForkSpec.history.height shouldEqual testSettings_version0.forkHeight + 1

    test_height = ForkSpec.history.height

  }

  property("Appending version3 blocks after height = forkHeight and then appending a version0 block should fail") {

    val tempBlock_version0 = BifrostBlock(ForkSpec.history.bestBlockId,
      System.currentTimeMillis(),
      ArbitBox(PublicKey25519Proposition(ForkSpec.history.bestBlockId), 0L, 10000L),
      Signature25519(Array.fill(Curve25519.SignatureLength)(1: Byte)),
      Seq(),
    ...
    val heightBeforeAppendAttempt = ForkSpec.history.height

    ForkSpec.history = ForkSpec.history.append(tempBlock_version0).get._1

    ForkSpec.history.modifierById(tempBlock_version0.id).isDefined shouldBe false

    val heightAfterAppendAttempt = ForkSpec.history.height
...
//    heightBeforeAppendAttempt shouldEqual heightAfterAppendAttempt
    ...
    ForkSpec.history.storage.rollback(tempBlock_version0.parentId)
    ForkSpec.history = new BifrostHistory(ForkSpec.history.storage,
      ForkSpec.testSettings_version3,
      Seq(
        new DifficultyBlockValidator(ForkSpec.history.storage)
        //new ParentBlockValidator(storage),
        //new SemanticBlockValidator(FastCryptographicHash)
      ))

    ForkSpec.history.height shouldEqual test_height

  }

  property("Appending version0 blocks after height = forkHeight should fail") {

    ForkSpec.history.storage.rollback(first_version3_block.parentId)
    ForkSpec.history = new BifrostHistory(ForkSpec.history.storage,
      ForkSpec.testSettings_version3,
      Seq(
        new DifficultyBlockValidator(ForkSpec.history.storage)
    ...
    ForkSpec.history.height shouldEqual testSettings_version0.forkHeight

    assert(ForkSpec.history.height == testSettings_version0.forkHeight)
