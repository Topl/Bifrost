...
object AugmentedBoxState {
  ...
  case class StateAugmentation(spentBoxIds: Set[Box.Id], newBoxIds: Set[Box.Id]) {
    ...
    def augment(transaction: IoTransaction): StateAugmentation = {
      val transactionSpentBoxIds = transaction.inputs.map(_.knownIdentifier.getTransactionOutput32).toSet
      val transactionId = transaction.id
      val transactionNewBoxIds =
        transaction.outputs.mapWithIndex((_, idx) => Box.Id(transactionId, idx.toShort)).toSet
      StateAugmentation(
